...
const ProfileCard = ({ user, allUsers, priority }: { user: User; allUsers: User[]; priority?: boolean }) => {
  ...
  return (
    <Link href={`/profile/${user.id}`} className="group block h-full active:scale-95 transition-transform">
    ...
    >
      <CarouselContent className="-ml-4">
        {shuffledUsers.map((user, index) => (
          <CarouselItem key={user.id} className="pl-3 basis-[82%] sm:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5">

...
export default function RelativeSelectionModal({
  ...
  const fuse = useMemo(() => new Fuse(sortedUsers, {
    keys: ['name', 'surname', 'family', 'fatherName'],
    threshold: 0.35,
  ...
  const filteredUsers = useMemo(() => {
    ...
    return fuse.search(searchTerm.trim(), { limit: 80 }).map(r => r.item);
  }, [fuse, sortedUsers, searchTerm]);

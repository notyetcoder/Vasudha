...
export default function UserFormModal({ isOpen, onClose, user, onSave, mode, prefillData }: UserFormModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            if (mode === 'edit' && user) {
                ...
                if (user.deathDate) {
                    try {
                        formattedDeathDate = format(new Date(user.deathDate), 'dd/MM/yyyy');
                    } catch (e) {
                       // ignore invalid date
                    }
                }
                ...
                setCurrentImageUrl(user.profilePictureUrl);
                 ...
                 reset({
                    ...emptyFormValues,
                    ...prefillData,
                    maidenName: prefillData?.surname || '',
    ...
    const onSubmit = async (values: FormData) => {
        ...
        let isoDeathDate: string | undefined = undefined;
        if (values.isDeceased && values.deathDate) {
            try {
                // Try to parse the date from DD/MM/YYYY format
                const parsedDate = parse(values.deathDate, 'dd/MM/yyyy', new Date());
                if (!isNaN(parsedDate.getTime())) {
                    isoDeathDate = parsedDate.toISOString();
                }
            } catch (e) {
                        ...
                        <div className="grid gap-2">
                            ...
                            <div className="flex items-center gap-2">
                                {pictureToDisplay && pictureToDisplay !== 'https://placehold.co/150x150.png' ? (
                                    <>
                                        <Image src={pictureToDisplay} alt="Profile" width={40} height={40} className="rounded-full" data-ai-hint="profile avatar" />
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <div className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor="isDeceased">Mark as Deceased</Label>
                                </div>
                            )}
                        />
                        {isDeceased && (

...
// Get the correct Gujarati sibling label using birth year comparison
function getSiblingLabel(root: User, sibling: User): string {
  const isRootOlder = isPerson1Older(root, sibling);
  if (sibling.gender === 'male') {
    if (isRootOlder === false) return 'મોટા ભાઈ';  // sibling is elder
    if (isRootOlder === true)  return 'નાના ભાઈ';  // sibling is younger
    return 'ભાઈ';
  } else {
    if (isRootOlder === false) return 'મોટી બહેન';
    if (isRootOlder === true)  return 'નાની બહેન';
    return 'બહેન';
  }
}
...
export function buildFamilyTree(rootId: string, allUsers: User[]): FamilyTree | null {
  ...
  for (const sib of elderSiblings) {
    const u = sib.user;
    const label = getSiblingLabel(self, u);
  ...
  // ── Generation 0: Self ────────────────────────────────────────────────────
  addNode(makeNode(self, 0, 'self', 'આપ (You)', true));

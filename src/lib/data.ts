...
// Export alias for backward compat
export const createPublicClient = createStaticClient;
...
// For the home page carousel — fetches users with photos, ordered randomly
// Uses created_at to get a diverse recent selection
export const getFeaturedUsers = async (limit: number = 60): Promise<User[]> => {
    const supabasePublic = createStaticClient();
    const { data, error } = await supabasePublic
        .from('users')
        .select('*')
        .not('profilePictureUrl', 'is', null)
        .not('profilePictureUrl', 'like', '%placehold%')
...
const uploadProfilePicture = async (supabase: SupabaseClient, userId: string, dataUri: string): Promise<string> => {
    ...
    // Return the clean public URL — next.config.js sets minimumCacheTTL=7days
    // The URL is unique per upload (upsert replaces the file in storage)
    // No timestamp needed — each upload produces the same URL which is fine
    // because the CDN/next.js image optimization caches it at the network level.

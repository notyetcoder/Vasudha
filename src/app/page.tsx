...
import { getFeaturedUsersAction } from '@/actions/users';
...
export default async function Home() {
    const { users: featuredUsers, allUsers } = await getFeaturedUsersAction(60);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background text-foreground">
            ...
            <main className="flex-1">
                <section className="flex flex-col items-center justify-center text-center pt-28 pb-16 px-4 container">
                    <h1 lang="hi" className="font-headline text-4xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-primary animate-fade-in [animation-delay:200ms]">
                        वसुधैव कुटुम्बकम्
                    </h1>
                    ...
                    <div className="mt-8 flex flex-col sm:flex-row items-center gap-3 animate-fade-in [animation-delay:600ms] w-full sm:w-auto px-4 sm:px-0">
                        <Button asChild size="lg" className="w-full sm:w-auto shadow-lg shadow-primary/20 h-12 text-base">
                            <Link href="/explore" aria-label="Explore the community family tree">
                                Explore The Community <ArrowRight className="ml-2 h-5 w-5" />
                            </Link>
                        </Button>
                        <Button asChild variant="outline" size="lg" className="w-full sm:w-auto bg-background/50 h-12 text-base">
                            <Link href="/relationships" aria-label="Find how two people are related">
                                <GitMerge className="mr-2 h-5 w-5" /> Find Connection
                            </Link>
                        </Button>
                        <Button asChild variant="outline" size="lg" className="w-full sm:w-auto bg-background/50 h-12 text-base">

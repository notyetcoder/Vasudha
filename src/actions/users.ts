...
import type { User, ActionResponse, CreateUserPayload } from '@/lib/types';
...
import {
  ...
  getFeaturedUsers as dbGetFeaturedUsers,
} from '@/lib/data';
...
export async function createUser(data: CreateUserPayload): Promise<ActionResponse> {
  try {
    // Basic input validation — defend against oversized or malicious payloads
    if (!data.name || !data.maidenName || !data.gender || !data.maritalStatus) {
      return { success: false, message: 'Required fields are missing.' };
    }
    if (typeof data.name === 'string' && data.name.length > 100) {
      return { success: false, message: 'Name is too long.' };
    }
    if (data.profilePictureUrl && typeof data.profilePictureUrl === 'string' && data.profilePictureUrl.length > 5_000_000) {
      return { success: false, message: 'Profile picture is too large. Please use a smaller image.' };
    }
...
// These actions are kept for backward compatibility but the profile page
// now uses pure in-memory lookups from server-utils.ts instead.
// These are only used if something still calls them directly.
export async function getFeaturedUsersAction(limit = 60): Promise<{ users: User[]; allUsers: User[] }> {
  try {
    // Fetch featured (with photos) + a broader pool for name lookups
    // Using 500 instead of 10k — enough to resolve spouse/parent names for carousel display
    const [featured, broader] = await Promise.all([
      dbGetFeaturedUsers(limit),
      dbGetAllUsersForPublic(1, 500),
    ]);
    return { users: featured, allUsers: broader.users };
  } catch (error) {
    console.error('getFeaturedUsers failed:', error);
...
export async function findChildrenAction(parentId: string): Promise<User[]> {
  try {
    const { users: allUsers } = await dbGetAllUsersForPublic(1, 10000);
    const { findChildren } = await import('@/lib/server-utils');
    return findChildren(parentId, allUsers);
  } catch (error) {
...
export async function findSiblingsAction(user: User): Promise<User[]> {
  try {
    const { users: allUsers } = await dbGetAllUsersForPublic(1, 10000);
    const { findSiblings } = await import('@/lib/server-utils');
    return findSiblings(user, allUsers);
  } catch (error) {
